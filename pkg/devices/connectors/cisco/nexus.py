...
class DeviceInfo(DeviceInfoPlugin):
    ...
    def __init__(self):
        ...
        self.user = credentials.get("nexus").get("username")
        self.password = credentials.get("nexus").get("password")
        self.port = credentials.get("nexus").get("port")
    ...
    def get_metadata_info(self, deviceInfo):
        ...
        try:
            ...
            status = ssh.connect(ip, self.port, self.user, self.password, timeout=5)
